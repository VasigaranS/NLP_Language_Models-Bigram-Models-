...
class BigramTrainer(object):
    ...
    def stats(self):
        ...
        for keys in self.unigram_count:
            rows_to_print.append(str(self.index[keys])+' '+str(keys)+' '+str(self.unigram_count[keys]))

        # YOUR CODE HERE
        

        for (m,n) in self.bigram_count:
            rows_to_print.append(str(m)+' '+str(n)+' '+str(format(math.log(self.bigram_count[(m,n)]/self.unigram_count[self.word[m]]),'.15f')))
